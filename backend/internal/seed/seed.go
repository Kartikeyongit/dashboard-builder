...
func RunSeed(db *sql.DB, encKey string) error {
	...
	if err != nil {
		return fmt.Errorf("encrypt devpass: %w", err)
	}

	exec := func(query string, args ...interface{}) {
		if _, e := db.Exec(query, args...); e != nil {
			log.Fatalf("Seed exec failed: %v\nQuery: %s", e, query)
		}
	}

	// 1. Base org / users / datasource
	...
	exec(`CREATE TABLE sample_customers (id SERIAL PRIMARY KEY, name TEXT, email TEXT, signup_date DATE, country TEXT)`)
	exec(`CREATE TABLE sample_products (id SERIAL PRIMARY KEY, name TEXT, category TEXT, price NUMERIC(10,2), stock INT)`)
	exec(`CREATE TABLE sample_orders (id SERIAL PRIMARY KEY, customer_id INT REFERENCES sample_customers(id), product_id INT REFERENCES sample_products(id), quantity INT, order_date TIMESTAMP, amount NUMERIC(10,2))`)
	exec(`CREATE TABLE sample_events (id SERIAL PRIMARY KEY, event_type TEXT, user_id INT, created_at TIMESTAMP, metadata JSONB)`)
	exec(`CREATE TABLE sample_metrics (id SERIAL PRIMARY KEY, metric_name TEXT, value DOUBLE PRECISION, recorded_at TIMESTAMP)`)
	...
	for i := 0; i < 200; i++ {
		...
		exec(`INSERT INTO sample_customers (name, email, signup_date, country) VALUES ($1,$2,$3,$4)`, name, email, signup.Format("2006-01-02"), country)
	}
	...
	for i := 1; i <= 50; i++ {
		...
		exec(`INSERT INTO sample_products (name, category, price, stock) VALUES ($1,$2,$3,$4)`, prodName, cat, price, stock)
	}
	...
	for i := 0; i < 1000; i++ {
		...
		exec(`INSERT INTO sample_orders (customer_id, product_id, quantity, order_date, amount) VALUES ($1,$2,$3,$4,$5)`, custID, prodID, qty, orderDate, amount)
	}
	...
	for i := 0; i < 500; i++ {
		...
		exec(`INSERT INTO sample_events (event_type, user_id, created_at, metadata) VALUES ($1,$2,$3,$4)`, eType, userID, t, meta)
	}
